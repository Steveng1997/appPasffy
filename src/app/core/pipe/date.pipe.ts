...
export class DatePipe implements PipeTransform {
  ...
  transform(items: any[], paramFechaInicial: string, paramFechaFinal: string): any {
    ...
    if (items) {
      ...
      if (paramFechaInicial === undefined) return items.filter((item) => item.dateToday.substring(0, 10) <= paramFechaFinal)
      if (paramFechaFinal === undefined) return items.filter((item) => item.dateToday.substring(0, 10) === paramFechaInicial)
